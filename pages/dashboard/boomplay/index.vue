  ...
  <div>
    <div>
      <div class="flex items-center w-full justify-between">
        <h3 class="text-3xl font-semibold">Boomplay Ads</h3>
        <NuxtLink
          class="bg-primary text-white rounded-md px-6 py-3"
          :to="{ name: 'dashboard-boomplay-create-update' }"
        >
      ...
      >
        ...
        <template #actions="{ item }">
          <div class="flex items-center gap-2">
            <NuxtLink
              :to="{
                name: 'dashboard-boomplay-create-update',
                query: { id: item._id },
              }"
              class="bg-primary text-white rounded-md px-6 py-3"
            >
...
export default {
  ...
  methods: {
    async fetchAds() {
      try {
        this.loading = true;
        const response = await fetch("/api/dashboard/boom-ads", {
          method: "GET",
          headers: {
    ...
    async deleteAd(id) {
      ...
      await fetch(`/api/dashboard/boom-ads/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Cookie: document.cookie,
    ...
    async toggle(item, index) {
      try {
        await fetch(`/api/dashboard/boom-ads/status/${item._id}`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Cookie: document.cookie,
